...
initiateStr="rs.initiate({
                 _id: '$replSetName',
                 members: [
                  { _id: 1, host: '$host:31330' , priority: 1},
                  { _id: 2, host: '$host:31331' },
                  { _id: 3, host: '$host:31332' }
                 ]
                })"
...
for ((i=0; i < ${#ports[@]}; i++))
do
  mongod --dbpath "$workingDir/r$i" --logpath "$workingDir/r$i/$logName" --port ${ports[$i]} --replSet $replSetName --keyFile "$workingDir/pki/m310-keyfile" --auditDestination file --auditFormat JSON --auditPath "$workingDir/r$i/$auditLog" --setParameter auditAuthorizationSuccess=true --fork
done
