...
root_ca_key="/home/vagrant/shared/certificate_sign/RootCA/root-ca.key"
signing_ca_key="/home/vagrant/shared/certificate_sign/SigningCA/signing-ca.key"
...
openssl ca -batch -notext -name RootCA -config $root_ca_config -extensions v3_req -keyfile $root_ca_key -out signing-ca.crt -infiles signing-ca.csr 
...
for host in "${mongodb_server_hosts[@]}"; do
    ...
    openssl ca -batch -notext -name SigningCA -config $root_ca_config -extensions v3_req -keyfile $signing_ca_key -out ${host}.crt -infiles ${host}.csr
...
openssl ca -batch -notext -name SigningCA -config $root_ca_config -extensions v3_req -keyfile $signing_ca_key -out client1.crt -infiles client1.csr
...
openssl ca -batch -notext -name SigningCA -config $root_ca_config -extensions v3_req -keyfile $signing_ca_key -out client2.crt -infiles client2.csr
